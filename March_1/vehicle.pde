...
class Vehicle {
  ...
  int CurrentDistance;
  float CurrentFear; //this is how much the fish is afraid at the moment
  float ThresholdFear;
...
void RunAway() {
  ...
  if (location.x < width/2) {
    CurrentFear = map(location.x, 0, width/2, 100, 0); // the canvas is 800 wide so i want to translate this into a value between 0 and 100 to half the canvas
    // this translates start of canvas to middle as 100 to 0
  } else {
    CurrentFear = map(location.x, width/2, width, 0, 100); // this translates middle of canvas to end as 0 to 100
  }

  if (location.y < height/2) {
    CurrentFear = map(location.y, 0, height/2, 100, 0);
  } else {
    CurrentFear = map(location.y, height/2, height, 0, 100);
  }


  // this is where we set what runaway does- compare current fear to threshold fear
  if (CurrentFear > ThresholdFear) {
    if (location.x > width/2) { //fish is closer to the right wall so it has to run to the left
      ...
      PVector running = new PVector( -1, 0 );
    } else {
      PVector running = new PVector( 1, 0 );
    }
    ...
    if (CurrentFear > ThresholdFear) {
      if (location.y > height/2) { //fish is closer to the right wall so it has to run to the left
        // we are trying to find an appropriate PVector to avoid the wall - some sort of force that guides the fish when it reaches threshold fear
        // when a fish runs into a wall it would run into the opposite direction, so if its the right wall the fish will simply run to the left
        PVector running = new PVector( 0, -1 );
      } else {
        PVector running = new PVector( 0, 1 );
      }
